...
class SurveyNLPAnalyzer:
    ...
    def generate_wordcloud_image(self, text_data, file_name, width=800, height=400):
        ...
        word_counts = Counter(text_data) # text_dataは単語リストとして受け取ることを想定
        wc.generate_from_frequencies(word_counts)

        # Matplotlibで描画し、それをStreamlitで表示するためにバイトデータとして取得
        fig, ax = plt.subplots(figsize=(width/100, height/100), dpi=100) # dpiでサイズ調整
        ax.imshow(wc, interpolation='bilinear')
        ax.axis("off")
        ax.set_title(file_name, fontproperties=font_prop, fontsize=18) # タイトルを引数から直接使用
        
        # Streamlitで表示するために図を返す
        return fig


    @st.cache_data # 分析結果をキャッシュして高速化
    def analyze_data(_self, df_input):
        ...
        analysis_results['overall']['top_words'] = Counter(overall_word_list).most_common(10)
        
        # ホバー情報表示のために、キーワードと元のテキスト、組織、性別を紐づけたDataFrameを作成
        keyword_data = []
                ...
                if preprocessed_text:
                    ...
                    for word in words:
                        keyword_data.append({
                            ...
                            '組織': row['組織'] if '組織' in _self.df.columns else 'N/A',
                            '性別': row['性別'] if '性別' in _self.df.columns else 'N/A',
                            'question': col
                        })
        ...
        for q_col in _self.question_cols:
            ...
            analysis_results['questions'][q_col]['top_words'] = Counter(q_word_list).most_common(10)
...
def main_app():
    ...
    if uploaded_file is not None:
        try:
            ...
            # 一般的な日本語CSVは 'shift_jis', 'cp932', 'utf-8' など
            ...
            if st.sidebar.button("分析を開始"):
                ...
                analysis_results, updated_df = analyzer.analyze_data(df) # キャッシュされた結果と更新されたdfを取得
            ...
            try:
                ...
                st.session_state['df_uploaded'] = df # 一時的に保存
            except Exception as e:
    ...
    if 'analysis_results' in st.session_state and 'analyzer' in st.session_state:
        ...
        with col2:
            ...
            if not overall_top_words_df.empty:
                ...
                # custom_dataを整形された文字列のリストとして生成
                overall_top_words_df['custom_hover_text'] = overall_top_words_df['キーワード'].apply(
                    lambda k: "<br>---<br>".join([
                        f"**回答**: {row['original_text']}<br>組織: {row['組織']}<br>性別: {row['性別']}{'<br>設問: ' + row['question'] if 'question' in row else ''}"
                ...
                st.write(overall_top_words_df[['キーワード', 'custom_hover_text']].head())
                # --- デバッグ用 ---

                fig_keywords = px.bar(overall_top_words_df, x='出現回数', y='キーワード', orientation='h', 
                                    title='頻出キーワード上位10 (全体)',
                                    # custom_dataに列名を文字列として直接指定
                                    custom_data=['custom_hover_text']) # <--- ここは列名（文字列）に戻す
                
                # ホバーテンプレートをカスタマイズ
                fig_keywords.update_traces(
                    # customdata[0]でアクセス
                    hovertemplate="<b>%{y}</b><br>出現回数: %{x}<br>" +
                                  "<b>関連する回答:</b><br>%{customdata[0]}" + # <--- ここも customdata[0] に戻す
                                  "<extra></extra>" 
                )
                
                st.plotly_chart(fig_keywords, use_container_width=True)
            else:
                st.info("頻出キーワードなし (全体)")
            ...
            with col_q1:
                ...
                if not q_top_words_df.empty:
                    ...
                    q_top_words_df['custom_hover_text'] = q_top_words_df['キーワード'].apply(
                        lambda k: "<br>---<br>".join([
                            f"**回答**: {row['original_text']}<br>組織: {row['組織']}<br>性別: {row['性別']}"
                            for _, row in detailed_df[detailed_df['keyword'] == k].iterrows()
                        ]) or "該当する回答がありません。"
                    )
                    ...
                    # st.write(q_top_words_df[['キーワード', 'custom_hover_text']].head())
                    # --- デバッグ用 ---

                    fig_q_keywords = px.bar(q_top_words_df, x='出現回数', y='キーワード', orientation='h',
                                            title=f'頻出キーワード上位10 ({q_col})',
                                            # custom_dataに列名を文字列として直接指定
                                            custom_data=['custom_hover_text']) # <--- ここも列名（文字列）に戻す
